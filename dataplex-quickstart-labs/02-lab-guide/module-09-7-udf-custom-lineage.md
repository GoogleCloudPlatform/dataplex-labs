# M9-7: UDF Custom Lineage with Custom Catalog Entry
...
CRIMES_UDF_DS="crimes_udf_ds"

```

### 1b. Create a "crimes_udf" BigQuery Dataset

```
bq --location=$LOCATION_MULTI mk \
    --dataset \
    $PROJECT_ID:$CRIMES_UDF_DS
```

### 1c. Load some data into the newly created "crimes udf" BigQuery dataset from a BigQuery public dataset

Paste this command in Cloud Shell to create a table-
```
bq --location=$LOCATION_MULTI query \
--use_legacy_sql=false \
"CREATE OR REPLACE TABLE $CRIMES_UDF_DS.crimes_staging AS SELECT * FROM bigquery-public-data.chicago_crime.crime"
```

### 1d. Create a UDF to transform the base table by aggregating

bq --location=$LOCATION_MULTI query \
--use_legacy_sql=false \
<<EOF
CREATE OR REPLACE FUNCTION $CRIMES_UDF_DS.concatenate_coordinates(x_coord FLOAT64, y_coord FLOAT64) AS (
  CONCAT(CAST(x_coord AS STRING), ':', CAST(y_coord AS STRING))
);
### 1e. Create a new table and use the udf
...
We first need to create a process. Lets create a fictitious process called "ELT UDF". Paste the below in cloud shell-
...
The process we created has the keyword "ETL", lets zone in:
```
cd ~/temp-lineage
cat lineage_processes_listing.json | grep -C 1 ETL
```
...
```
CUSTOM_LINEAGE_PROCESS_ID=`cat lineage_processes_listing.json | grep -C 1 ETL | grep name | cut -d'/' -f6 | tr -d \" | tr -d ,`
...
We will create an execution instance of the "ETL UDF" process we created, and refer to it as a "run". 
