...
from utils import logging_utils, file_utils, business_glossary_utils
from utils.constants import DP_LINK_TYPE_DEFINITION
...
def get_link_type(file_path: str) -> str | None:
    data = file_utils.read_first_json_line(file_path)
    if data:
...
def extract_scopes_from_entry_references(data: dict) -> set:
    ...
    logger.debug(f"Extracting scopes from entry references: {entry_references}")
    for ref in entry_references[:2]:
        name = ref.get("name", "")
    ...
    try:
        with open(file_path, 'r', encoding='utf-8') as f:
            for line in f:
                if not line.strip():
                    continue
                data = file_utils.parse_json_line(line)
                if data:
                    scopes.update(extract_scopes_from_entry_references(data))
    except IOError:
        pass
    logger.debug(f"Project scopes from all lines in {file_path}: {scopes}")
    return scopes
...
def build_related_synonym_referenced_entry_scopes(file_path: str, main_project_id: str) -> List[str]:
    logger.debug(f"Building referenced scopes for file: {file_path} and main_project_id: {main_project_id}")
    scopes = get_project_scopes_from_all_lines(file_path)
    scopes.add(f"projects/{main_project_id}")
    logger.debug(f"Built scopes: {scopes}")
    return list(scopes)
...
def build_definition_entrylink_payload(file_path: str, project_id: str, import_spec_base: dict) -> Tuple[str, dict, str]:
    ...
    entry_group = file_utils.get_entry_group(file_path)
    job_location = extract_job_location_from_entry_group(entry_group)
    job_id_prefix = business_glossary_utils.normalize_id(filename)
        ...
        "import_spec": {
            **import_spec_base,
            "scope": {
                ...
                "entry_link_types": [f"projects/dataplex-types/locations/global/entryLinkTypes/definition"],
...
def build_synonym_related_entrylink_payload(file_path: str, project_id: str, import_spec_base: dict) -> Tuple[str, dict, str]:
    ...
    payload = {
        ...
        "import_spec": {
            **import_spec_base,
            "scope": {
                "entry_groups": [f"projects/{project_id}/locations/{job_location}/entryGroups/@dataplex"],
                "entry_link_types": [
...
def build_entrylink_payload(file_path: str, project_id: str, import_spec_base: dict) -> Tuple[Optional[str], Optional[dict], Optional[str]]:
    ...
    if DP_LINK_TYPE_DEFINITION in link_type:
