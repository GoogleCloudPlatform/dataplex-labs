...
def perform_imports(project_id: str, buckets: list[str]):
    ...
    try:
        business_glossary_import_v2.main(project_id, buckets)
    except Exception as e:
...
def export_and_validate_glossaries(project_id: str, user_project: str, org_ids: list[str], start_time: float) -> bool:
    """Finds, exports, and validates business glossaries."""
...
def main(args: argparse.Namespace) -> None:
    ...
    if not args.resume_import:
        export_and_validate_glossaries(project_id, user_project, org_ids, start_time)

    perform_imports(project_id, buckets)
