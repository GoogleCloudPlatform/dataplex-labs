...
TAG_TEMPLATE = models.Variable.get('tag_template_data_product_info')
TAG_INPUT_FILE = models.Variable.get('merchant_dp_info_input_file')
TAG_INPUT_PATH = models.Variable.get('merchant_dp_info_input_path')
SUB_NETWORK = models.Variable.get('gcp_sub_net')
TAG_JAR = models.Variable.get('gdc_tag_jar')
DPLX_TASK_PREFIX = "merchant-dp-info-tag"
    ...
    # )
        ...
        create_dataplex_dp_info_tag_task = DataplexCreateTaskOperator(
            ...
            body={
                ...
                "spark": {
                    "main_class": "com.google.cloud.dataplex.templates.dataproductinformation.DataProductInfo",
        ...
        dataplex_task_state = BranchPythonOperator(
            ...
            op_kwargs={
                'dplx_task_id': f"{DPLX_TASK_PREFIX}-{{{{ ti.xcom_pull(task_ids='{pre_task_id}', key='return_value') }}}}", 'entity_val': f"{entity}"}
        ...
        start >> generate_uuid_dq_check >> create_dataplex_dp_info_tag_task >> dataplex_task_state >> [
            dataplex_task_success, dataplex_task_failed] >> end
