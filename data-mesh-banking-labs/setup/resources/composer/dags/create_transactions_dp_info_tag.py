...
TAG_INPUT_FILE = models.Variable.get('transactions_dp_info_input_file')
TAG_INPUT_PATH = models.Variable.get('transactions_dp_info_input_path')
SUB_NETWORK=models.Variable.get('gcp_sub_net')
TAG_JAR=models.Variable.get('gdc_tag_jar')
DPLX_TASK_PREFIX="trans-dp-info-tag"
...
def _get_dataplex_job_state(**kwargs):
    ...
    while (task_status != 'SUCCEEDED' and task_status != 'FAILED'):
        ...
        time.sleep(30)
        task_status = get_clouddq_task_status(kwargs['dplx_task_id'])
        print(f"Cloud Dplx  task status is {task_status}")
