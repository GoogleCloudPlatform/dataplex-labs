...
TAG_TEMPLATE = models.Variable.get('tag_template_data_product_exchange') 
TAG_INPUT_FILE = models.Variable.get('customer_dx_input_file')
TAG_INPUT_PATH = models.Variable.get('customer_dx_info_input_path')
SUB_NETWORK=models.Variable.get('gcp_sub_net')
TAG_JAR=models.Variable.get('gdc_tag_jar')
DPLX_TASK_PREFIX="airflow-cust-dxtag"
TAG_MAIN_CLASS=models.Variable.get('data_exchange_main_class')
...
def _get_dataplex_job_state(**kwargs):
    ...
    while (task_status != 'SUCCEEDED' and task_status != 'FAILED'):
        ...
        time.sleep(30)
        task_status = get_clouddq_task_status(kwargs['dplx_task_id'])
        print(f"Cloud Dataplex task status is {task_status}")
    return task_status+ "_{}".format(kwargs['entity_val'])


with models.DAG(
        'data_governance_customer_exchange_tag',
